...
<body>
	...
	<div class="container">
		<div align="center" class="titulo">
			<span class="titulorel">Relatório de Investimentos</span>
		</div>
		...
		<table class='table' width='100%' cellspacing='0' cellpadding='3'>
			...
			<?php
			$query = $pdo->query("SELECT * FROM investimentos where data >= '$dataInicial' and data <= '$dataFinal' and id_usuario = '$usu' order by $ordenacao ");
			...
			for ($i = 0; $i < @count($res); $i++) {
				...
				$nome = $res[$i]['nome'];
				$data = $res[$i]['data'];
				$data_investimento_formatada = implode('/', array_reverse(explode('-', $res[$i]['data'])));;
				$tipo_invesimento = $res[$i]['tipo_investimento'];
				$valor = $res[$i]['valor'];
				...
				$query2 = $pdo->query("SELECT * FROM tipo_investimento where id = '$tipo_invesimento'");
			...
			?>
				...
				<tr class="">
					<td><?php echo $nome ?> </td>
					<td><?php echo $data_investimento_formatada ?> </td>
					<td><?php echo $nome_tipo_investimento ?> </td>
					<td>R$ <?php echo $valorFormatado?></td>
				</tr>
			<?php } ?>
